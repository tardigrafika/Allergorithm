"""
LOCO na SAMO Confirmed+Strong evidence tier, sa PU bagging-om - 1512 dataset.

Popravka metodoloske rupe u loco_confirmed_strong_1512.py: taj skript je
za trening koristio JEDNO nasumicno izvlacenje "negativa" (single-draw RF),
iako smo ranije u sesiji zakljucili da je missing-negatives problem
principijelno PU (positive-unlabeled) problem, i da ga PU bagging resava
usrednjavanjem preko mnogo nezavisnih izvlacenja. Odluka da se PU bagging
izostavi iz LOCO/Confirmed+Strong testova bila je opravdana ranijim K=5
nalazom "PU ~= RF+BLAST" - ali taj K=5 test je imao SLABU stat. snagu (to je
bas problem koji LOCO resava), pa nije validno oslanjanje za odluku u
NAJPRECIZNIJEM testu koji imamo. Rezultat: "RF+BLAST ne prelazi cosine na
Confirmed+Strong" mogao je biti artefakt jednog nesrecnog izvlacenja
negativa, ne prava istina.

Dodaje PU bagging (20 bagova, isti pristup kao random_forest_pu_bagging_1443.py)
uz vec postojeci cosine i single-draw RF+BLAST, da se vidi da li usrednjavanje
preko negativnih izvlacenja menja zakljucak.

Izlaz:
    output/loco_confirmed_strong_pu_1512_per_fold.csv
    output/loco_confirmed_strong_pu_1512_summary.txt
"""
...
PER_FOLD_OUTPUT = OUTPUT_DIR / "loco_confirmed_strong_pu_1512_per_fold.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "loco_confirmed_strong_pu_1512_summary.txt"
...
gold_all = gold_raw.loc[~negative_mask].copy()
print(f"All positive gold-standard pairs (any tier): {len(gold_all)}")

confirmed_strong_mask = gold_all["evidence_level"].str.startswith(("Confirmed", "Strong evidence"), na=False)
gold = gold_all.loc[confirmed_strong_mask].copy()
print(f"Confirmed+Strong subset (used for train/test): {len(gold)}")
...
gold_pairs_all = map_pairs(gold_all)
gold_pairs = map_pairs(gold)
print(f"Mapped gold pairs (all tiers, for negative exclusion): {len(gold_pairs_all)}")
print(f"Mapped gold pairs (Confirmed+Strong, train/test/components): {len(gold_pairs)}")

# negative sampling must avoid EVERY known positive (even weak Inferred-tier
# ones we're not training/testing on here) -- a weakly-documented pair is
# still not a safe negative
positive_pair_set = {tuple(sorted((p["id_1"], p["id_2"]))) for p in gold_pairs_all}
...
all_cos_rr, all_rf_rr, all_pu_rr = [], [], []
...
for fold_idx, held_out in enumerate(component_list):
    ...
    # single-draw RF+BLAST (kept for comparison with the earlier result)
    ...
    # PU bagging: N_BAGS models, each with its OWN fresh negative draw
    pu_models = []
    ...
    fold_row = {
        ...
        "pu_bagging_mrr": float(np.mean(pu_rr)) if pu_rr else float("nan"),
    }
    for k in TOP_K:
        ...
        fold_row[f"pu_bagging_hits_at_{k}"] = float(np.mean(pu_hits[k])) if pu_hits[k] else float("nan")
    ...
    if (fold_idx + 1) % 5 == 0 or (fold_idx + 1) == K_FOLDS:
        ...
        print(f"  fold {fold_idx + 1}/{K_FOLDS} done (size={len(held_out)}, queries={len(cos_rr)}) "
              ...
              f"pu_bagging={fold_row['pu_bagging_mrr']:.4f} ({elapsed/60:.1f} min elapsed)")
...
pu_macro = per_fold_df["pu_bagging_mrr"].to_numpy()
...
summary_lines.append(f"  PU bagging   MRR: {pu_micro:.4f}")
summary_lines.append("")
summary_lines.append(f"Paired per-component wins (RF+BLAST > cosine): {wins_rf}/{K_FOLDS} components")
summary_lines.append(f"Paired per-component wins (PU bagging > cosine): {wins_pu}/{K_FOLDS} components")
mean_delta_rf = float(paired_delta_rf.mean())
se_delta_rf = float(paired_delta_rf.std(ddof=1) / np.sqrt(K_FOLDS))
mean_delta_pu = float(paired_delta_pu.mean())
se_delta_pu = float(paired_delta_pu.std(ddof=1) / np.sqrt(K_FOLDS))
summary_lines.append(f"Mean paired delta (RF+BLAST - cosine): {mean_delta_rf:+.4f} (SE {se_delta_rf:.4f})")
summary_lines.append(f"Mean paired delta (PU bagging - cosine): {mean_delta_pu:+.4f} (SE {se_delta_pu:.4f})")
if abs(mean_delta_pu) > 2 * se_delta_pu:
    verdict = "PU bagging delta is more than 2 SE from zero -- looks like a REAL, statistically distinguishable effect."
else:
    verdict = "PU bagging delta is within ~2 SE of zero -- still not statistically distinguishable from noise."
