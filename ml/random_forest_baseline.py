...
Cilj: Proveriti da li Random Forest može bolje da predvidi cross-reactivity od cosine similarity baseline-a.

Feature-i: apsolutna razlika ESM-2 embeddinga + cosine similarity = 1281 feature
Pozitivni podaci: 296 poznatih cross-reactive parova
Negativni podaci: nasumično izabrani negativni parovi, odnos 10:1
Podela podataka: protein-level split bez data leakage-a; povezani proteini se drže u istom train/test skupu
Model: Random Forest.
Evaluacija: Hits@1, Hits@5, Hits@10, Hits@20 i MRR
Retrieval: svaki test protein se poredi sa svih 1534 proteina, isto kao kod cosine baseline-a, radi fer poređenja1

Izlazi:

random_forest_model.joblib
random_forest_retrieval_results.csv
...
MODEL_OUTPUT = OUTPUT_DIR / "random_forest_model.joblib"
RETRIEVAL_OUTPUT = OUTPUT_DIR / "random_forest_retrieval_results.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "random_forest_summary.txt"
...
SEED = 42               # single fixed seed used everywhere below
TEST_FRACTION = 0.2      # target fraction of proteins held out for testing
NEG_PER_POS = 10          # negative:positive sampling ratio (train and test)
TOP_K = [1, 5, 10, 20]

RF_PARAMS = dict(
    n_estimators=300,
    max_depth=12,          # shallow trees: 1281 features, only ~2600 training rows
    min_samples_leaf=3,
...
# (identical logic to analysis/hitsk_and_mrr.py, reused for consistency)
# =====================================================
...
# =====================================================
# EMBEDDING MATRIX (full candidate universe, same order used
# throughout for both feature construction and retrieval)
# =====================================================
...
# Precomputed full cosine similarity matrix, identical definition to
# analysis/hitsk_and_mrr.py. Used below to score the cosine baseline on
# exactly the same test-split queries the Random Forest is evaluated on,
# so the two methods are compared on an identical query set (not just an
...
gold_pairs = []  # list of dicts: id_1, id_2, pair_id, family_1, family_2
missing_pairs = 0
...
# global set of known positive pairs (order-independent), used to make sure
# negative sampling can never accidentally draw a documented positive pair
positive_pair_set = {
...
# (connected components of the gold-standard graph are kept intact)
# =====================================================
...
# 1) assign whole components to train/test, largest-effort match to TEST_FRACTION
...
# 2) independently split proteins with no gold-standard edges
...
# =====================================================
# NEGATIVE SAMPLING (reproducible, split-local, leakage-safe)
# =====================================================

def sample_negative_pairs(protein_pool, n_needed, seed):
    """
    Uniformly sample random protein pairs from `protein_pool` that are
    not documented cross-reactive pairs (checked against the GLOBAL
    positive set, in both directions). Sampling is restricted to a
    single split's protein pool, so negatives never mix train/test
    proteins.
    """
    local_rng = np.random.default_rng(seed)
    # sorted(), not list(): Python randomizes string-hash order per process,
    # so list(some_set) is NOT reproducible across runs even with a fixed
    # numpy seed -- sorting pins down a deterministic element order.
    ...
    # sorted(), not list(): tuple/string hash order is randomized per
    # process too, so this pins down a deterministic row order for the
    # resulting feature matrix (RF's bootstrap sampling is index-based,
    # so row order otherwise silently changes which trees get trained).
...
def pairwise_features(emb_a, emb_b):
    """
    Symmetric pairwise feature vector(s) for protein pair(s) (A, B).

    emb_a, emb_b: arrays of shape (N, 1280) or (1280,), row-aligned.
    Returns an (N, 1281) array: [abs(A - B), cosine_similarity(A, B)].

    Both components are invariant to swapping A and B, so the model
    treats (A, B) and (B, A) identically, matching the symmetric
    nature of "cross-reactive" as a relation.
    """
...
# =====================================================
# B) RETRIEVAL EVALUATION (directly comparable to cosine baseline)
# =====================================================
...
print("Both Random Forest and cosine similarity are scored on the exact "
      "same queries below, for a like-for-like comparison.")
...
for p in test_positive_pairs:
    ...
    for query_id, target_id, query_name, target_name, family_q, family_t in directions:
        ...
        rf_scores[query_index] = -np.inf  # exclude self
        ...
        retrieval_results.append(
            {
                ...
                "cosine_similarity": cos_true_pair_similarity,
...
print(f"{'Metric':<10}{'Cosine (same 55)':<20}{'Random Forest':<20}")
for k in TOP_K:
...
summary_lines.append("RANDOM FOREST BASELINE - SUMMARY")
summary_lines.append("=" * 60)
...
summary_lines.append("Split strategy: group-aware, protein-level split.")
summary_lines.append("Connected components of the gold-standard cross-reactivity")
summary_lines.append("graph are kept intact and assigned whole to one split, so no")
summary_lines.append("protein with a documented cross-reactive partner appears in")
summary_lines.append("both train and test. Proteins with no gold-standard edges are")
summary_lines.append("split independently with the same seed.")
...
header = f"{'Metric':<10}{'Cosine (full 296)':<20}{'Cosine (same 55)':<20}{'Random Forest':<20}"
summary_lines.append(header)
summary_lines.append("-" * len(header))
for k in TOP_K:
    summary_lines.append(
        f"{'Hits@' + str(k):<10}{cosine_hits[k]:<20.4f}"
        f"{cosine_test_hits[k]:<20.4f}{rf_hits[k]:<20.4f}"
    )
summary_lines.append(
    f"{'MRR':<10}{cosine_mrr:<20.4f}{cosine_test_mrr:<20.4f}{rf_mrr:<20.4f}"
)

summary_lines.append(
    "\nNOTE: 'Cosine (full 296)' is the original benchmark over all 592 "
    "queries (analysis/hitsk_and_mrr.py / output/hits_mrr_results.csv), "
    "kept here for context only -- it is NOT a fair comparison to Random "
    "Forest because it includes queries the Random Forest was trained on. "
    "'Cosine (same 55)' recomputes the cosine baseline on the exact same "
    f"{len(retrieval_df)} held-out test queries used to evaluate the "
    "Random Forest, with the same candidate pool (all 1534 proteins) and "
    "the same self-exclusion rule -- this is the correct like-for-like "
    "comparison."
)

if not cosine_available:
    summary_lines.append(
        f"\nNOTE: {COSINE_RESULTS} not found -- 'Cosine (full 296)' column is NaN."
    )
