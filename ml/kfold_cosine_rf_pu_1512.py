"""
K-fold unakrsna validacija: Cosine vs RF+BLAST vs PU bagging - 1512 dataset
(prosireni gold standard posle rundi kuracije: LTP sindrom, PR-10, tropomiozini,
serumski albumini, polkalcini, TLP -- vidi output/cross_reactive_1512.csv).

Identican protokol kao ml/kfold_cosine_rf_pu_1443.py (isti feature-i, isti
RF/PU bagging hiperparametri) -- jedina razlika je ulazni gold-standard fajl.
Direktno poredi da li je porast broja nezavisnih povezanih komponenti (29 -> 36
posle ove runde kuracije) smanjio fold-to-fold varijansu koju smo izmerili na
1443 dataset-u (MRR std 0.036-0.039, veca od razlike izmedju metoda).
...
para), pa razlika od par hiljaditih MRR moze biti sum konkretnog split-a, ne
stvaran signal. Ovaj skript to proverava.
...
Ponavlja se na K=5 nezavisnih, protein-level (group-aware) foldova - isti
Union-Find leakage-prevention princip kao svuda (nijedan gold par ne sme da
predje granicu fold-a). K=5 umesto 10: gold graf ima samo 29 povezanih
komponenti sa >=1 pozitivnim parom - sa 10 foldova bi u proseku bilo ~2-3
komponente po foldu, premalo za stabilnu procenu.
...
BLAST_MATRIX = Path("/home/lana/ALERGRAF/output/blast_identity_matrix_1443.pkl")  # vec pokriva svih 316 proteina (proveren)

OUTPUT_DIR = Path("/home/lana/ALERGRAF/output")
PER_FOLD_OUTPUT = OUTPUT_DIR / "kfold_comparison_1512_per_fold.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "kfold_comparison_1512_summary.txt"
