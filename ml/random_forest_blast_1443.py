"""
Random Forest sa dodatim BLAST-style feature-ima (% identity, alignment score) - 1443 dataset.

Ne menja random_forest_baseline_1443.py (bez BLAST-a ostaje odvojena, reproducibilna verzija).
Isti split, isti negative sampling, isti RF hiperparametri - samo se feature vektor
prosiruje sa 1281 na 1283 dimenzije (abs_diff[1280] + cosine[1] + blast_identity[1] + blast_score[1]).

Zasto ovo ima smisla kad cosine+RF ensemble nije pomogao: BLAST identity NIJE izveden
iz ESM embeddinga - to je potpuno nezavisan signal (klasicno poravnanje sekvenci,
BLOSUM62), za razliku od cosine-a koji RF vec implicitno sadrzi. Bio je i eksplicitno
trazen u plan/firststeps.md kao baseline za poredjenje, nikad ranije implementiran.

BLAST matrica racunata unapred (data/compute_blast_identity_1443.py, Biopython
lokalno poravnanje - blastp binarni alat nije dostupan u ovom okruzenju).

Izlaz:
    output/random_forest_blast_model_1443.joblib
    output/random_forest_blast_retrieval_results_1443.csv
    output/random_forest_blast_summary_1443.txt
"""
...
MODEL_OUTPUT = OUTPUT_DIR / "random_forest_blast_model_1443.joblib"
RETRIEVAL_OUTPUT = OUTPUT_DIR / "random_forest_blast_retrieval_results_1443.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "random_forest_blast_summary_1443.txt"
...
# (everything else identical to ml/random_forest_baseline_1443.py's
# pairwise_features; the two new scalar columns are simply appended)
# =====================================================

def blast_features(id_a, id_b):
    ia, ib = blast_id_to_index[id_a], blast_id_to_index[id_b]
    return blast_identity_matrix[ia, ib], blast_score_matrix[ia, ib]
...
print("\n==============================")
print("BUILDING FEATURE MATRICES (1281 + blast_identity + blast_score = 1283 dims)")
print("==============================")
...
print("\n==============================")
print("TRAINING RANDOM FOREST (with BLAST features)")
print("==============================")
...
# feature importance sanity check: where do the 2 new BLAST columns rank?
importances = rf.feature_importances_
blast_identity_rank = int((importances > importances[-2]).sum()) + 1
blast_score_rank = int((importances > importances[-1]).sum()) + 1
print(f"\nBLAST identity feature importance: {importances[-2]:.5f}  (rank {blast_identity_rank}/{len(importances)})")
print(f"BLAST score feature importance   : {importances[-1]:.5f}  (rank {blast_score_rank}/{len(importances)})")
print(f"(for reference, mean importance across all {len(importances)} features: {importances.mean():.5f})")
...
print(f"{'Metric':<10}{'Cosine (same test)':<20}{'RF+BLAST':<20}")
for k in TOP_K:
...
# =====================================================
# COMPARISON WITH BASELINE (ESM-only) RF
# =====================================================

baseline_available = BASELINE_RF_RESULTS.exists()
baseline_hits = {k: float("nan") for k in TOP_K}
baseline_mrr = float("nan")
split_consistent = None

if baseline_available:
    baseline_df = pd.read_csv(BASELINE_RF_RESULTS)
    if len(baseline_df) == len(retrieval_df):
        ...
        baseline_hits = {k: baseline_df[f"rf_hits_at_{k}"].mean() for k in TOP_K}
        baseline_mrr = baseline_df["rf_reciprocal_rank"].mean()

print(f"\nBaseline RF (ESM-only) results file found: {baseline_available}")
if baseline_available:
    print(f"Split consistency check vs baseline RF script: {split_consistent}")
...
summary_lines.append("RANDOM FOREST + BLAST IDENTITY (1443 dataset) - SUMMARY")
summary_lines.append("=" * 60)
...
summary_lines.append(f"Feature vector: 1280 (abs_diff) + cosine + blast_identity + blast_score = "
                      f"{X_train.shape[1]} dims")
summary_lines.append(f"BLAST identity feature importance: {importances[-2]:.5f} (rank {blast_identity_rank}/{len(importances)})")
summary_lines.append(f"BLAST score feature importance   : {importances[-1]:.5f} (rank {blast_score_rank}/{len(importances)})")
...
if split_consistent is not None:
    summary_lines.append(f"  Split consistency check vs baseline RF: {split_consistent}")
summary_lines.append("")
...
header = f"{'Metric':<10}{'Cosine (same test)':<20}{'RF (ESM-only)':<20}{'RF+BLAST':<20}"
summary_lines.append(header)
summary_lines.append("-" * len(header))
for k in TOP_K:
    summary_lines.append(
        f"{'Hits@' + str(k):<10}{cosine_test_hits[k]:<20.4f}"
        f"{baseline_hits[k]:<20.4f}{rf_hits[k]:<20.4f}"
    )
summary_lines.append(
    f"{'MRR':<10}{cosine_test_mrr:<20.4f}{baseline_mrr:<20.4f}{rf_mrr:<20.4f}"
)

if baseline_available:
    delta_mrr = rf_mrr - baseline_mrr
    summary_lines.append(f"\nDelta vs baseline RF (ESM-only): MRR {delta_mrr:+.4f}")
    verdict = "IMPROVED" if delta_mrr > 0 else ("WORSE" if delta_mrr < 0 else "UNCHANGED")
    summary_lines.append(f"Adding BLAST identity/score {verdict} Random Forest retrieval on this dataset.")
else:
    summary_lines.append(f"\nNOTE: {BASELINE_RF_RESULTS} not found -- run ml/random_forest_baseline_1443.py first.")
