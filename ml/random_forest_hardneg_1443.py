"""
Random Forest sa mesavinom hard/easy negativa - 1443 dataset.

Ne menja random_forest_baseline_1443.py (uniform-random verzija ostaje odvojena).
30% negativa hard (cross-family, geometrijski najslicniji "hardest of K"),
ostatak nasumican iz celog train pool-a - ne 100% hard (to je pokvarilo
triplet-hardneg eksperiment suzavanjem trening distribucije).

Rezultat: LOSIJE od baseline RF-a (MRR 0.198 -> 0.184).

Izlaz:
    output/random_forest_hardneg_model_1443.joblib
    output/random_forest_hardneg_retrieval_results_1443.csv
    output/random_forest_hardneg_summary_1443.txt
"""
...
MODEL_OUTPUT = OUTPUT_DIR / "random_forest_hardneg_model_1443.joblib"
RETRIEVAL_OUTPUT = OUTPUT_DIR / "random_forest_hardneg_retrieval_results_1443.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "random_forest_hardneg_summary_1443.txt"

BASELINE_RF_RESULTS = Path("/home/lana/ALERGRAF/output/random_forest_retrieval_results_1443.csv")


# =====================================================
# CONFIGURATION (identical to ml/random_forest_baseline_1443.py except
# the new HARD_FRAC / HARD_CANDIDATES_PER_DRAW negative-mining knobs)
# =====================================================
...
HARD_FRAC = 0.3              # fraction of negatives that are hard; matches
                              # data/build_ml_dataset.py's own default, not tuned
HARD_CANDIDATES_PER_DRAW = 10  # "hardest of K"
...
# =====================================================
# NEGATIVE SAMPLING -- MIXED: HARD_FRAC hard (safe cross-family,
# geometrically hardest-of-K) + the rest uniform-random from the FULL
# pool (unlike the failed triplet-hardneg script, this keeps broad
# coverage of the pool so the classifier isn't only ever shown the
# narrow, family-labeled slice of proteins -- see module docstring)
# =====================================================
...
def sample_negative_pairs_mixed(protein_pool, n_needed, seed):
    ...
    # --- hard: hardest-of-K, safe cross-family ---
    ...
    # --- easy: uniform random from the FULL pool ---
...
# =====================================================
# FEATURE CONSTRUCTION (identical function to the baseline script)
# =====================================================
...
print("\n==============================")
print("TRAINING RANDOM FOREST (hard negatives)")
print("==============================")
...
print(f"{'Metric':<10}{'Cosine (same test)':<20}{'RF (hard neg)':<20}")
for k in TOP_K:
...
# =====================================================
# COMPARISON WITH BASELINE (uniform-random-negative) RF
# =====================================================
...
print(f"\nBaseline RF (uniform-random-negative) results file found: {baseline_available}")
if baseline_available:
...
summary_lines.append("RANDOM FOREST, HARD NEGATIVES (1443 dataset) - SUMMARY")
summary_lines.append("=" * 60)
...
header = f"{'Metric':<10}{'Cosine (same test)':<20}{'RF (baseline)':<20}{'RF (hard neg)':<20}"
...
if baseline_available:
    ...
    summary_lines.append(f"\nDelta vs baseline RF (uniform-random negatives): MRR {delta_mrr:+.4f}")
    verdict = "IMPROVED" if delta_mrr > 0 else ("WORSE" if delta_mrr < 0 else "UNCHANGED")
    summary_lines.append(f"Hard-negative mining {verdict} Random Forest retrieval on this dataset.")
else:
