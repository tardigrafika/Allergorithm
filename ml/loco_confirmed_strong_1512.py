"""
LOCO na SAMO Confirmed+Strong evidence tier - 1512 dataset.

Dijagnostika: LOCO na punom skupu (svih 1499 pozitiva, uklj. 1112 slabih
"Inferred family-level" parova) je pokazao da RF+BLAST ne prelazi cosine
(micro MRR 0.1153 vs 0.1163 - u sumu). Pitanje: da li slab Inferred tier
gusi stvaran signal koji RF ima na kvalitetnijim labelama?

Trenira i evaluira ISKLJUCIVO na Confirmed+Strong parovima (289 od 1499,
evidence_level pocinje sa "Confirmed" ili "Strong evidence"). Negative
sampling i dalje izbegava CEO skup od 1499 poznatih pozitiva (i slabe
Inferred parove) - ne zelimo da nasumicno "negativan" par ispadne poznat,
makar i slabo dokumentovan, pozitiv.

Confirmed+Strong podgraf ima VISE komponenti (47) nego pun graf (36) iako
ima mnogo manje parova (289 vs 1499) - Inferred-tier parovi ocigledno
premoscuju/spajaju klastere koji bi inace bili odvojeni.

Izvestava macro i micro MRR (isto kao loco_cosine_rf_blast_1512.py) - micro
(query-weighted) je pouzdaniji broj za zakljucak.

Izlaz:
    output/loco_confirmed_strong_1512_per_fold.csv
    output/loco_confirmed_strong_1512_summary.txt
"""
...
PER_FOLD_OUTPUT = OUTPUT_DIR / "loco_confirmed_strong_1512_per_fold.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "loco_confirmed_strong_1512_summary.txt"
...
summary_lines.append(f"LOCO Confirmed+Strong only ({K_FOLDS} folds): Cosine vs RF+BLAST (1512 dataset)")
summary_lines.append("=" * 70)
