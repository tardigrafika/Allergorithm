"""
ristup B: MLP transformacija embedding prostora za predikciju cross-reactivity alergena pomocu ESM2

embedding_A (1280) > MLP > predvidjeni embedding_B (1280)

Koristi samo 296 pozitivnih gold-standard parova, bez negative sampling-a
Svaki train par daje 2 primera A > B i B > A
Isti group-aware protein-level split kao kod RF i MLP classifier-a
55 test parova / 110 retrieval upita
Dodatni 85/15 pair-level split koristi se samo za early stopping
Embeddinge standardizuje samo na train proteinima, bez leakage-a
MLP 1280 → 256 → 128 → 1280, uz ReLU i Dropout
Trening pomocu MSELoss
Zbog malog broja podataka postoji veliki rizik od overfittinga
Koriste se dropout, weight decay i early stopping
Radi se i sanity check u odnosu na jednostavan identity baseline
Za svaki test upit model predvidja embedding partnera, zatim se racuna cosine similarity prema svih 1.534 proteina, bez self-matcha
Rezultati se mere pomocu Hits@K i MRR

Ulaz:
    embeddings/embeddings.pkl, embeddings/embeddings.parquet,
    output/cross_reactive_combined.csv
    output/random_forest_retrieval_results.csv   (RF's saved results, reused)
    output/mlp_retrieval_results.csv             (MLP classifier's saved results, reused)

Izlaz:
    output/mlp_embedding_transform_model.pt
    output/mlp_embedding_transform_retrieval_results.csv
    output/mlp_embedding_transform_summary.txt
"""
...
MLP_CLF_RETRIEVAL_RESULTS = Path("/home/lana/ALERGRAF/output/mlp_retrieval_results.csv")

OUTPUT_DIR = Path("/home/lana/ALERGRAF/output")
MODEL_OUTPUT = OUTPUT_DIR / "mlp_embedding_transform_model.pt"
RETRIEVAL_OUTPUT = OUTPUT_DIR / "mlp_embedding_transform_retrieval_results.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "mlp_embedding_transform_summary.txt"
...
SEED = 42
TEST_FRACTION = 0.2       # protein-level split, identical to RF/MLP classifier
VAL_FRACTION = 0.15        # PAIR-level carve-out from TRAIN, for early stopping
TOP_K = [1, 5, 10, 20]
...
PATIENCE = 30
LEARNING_RATE = 1e-3
WEIGHT_DECAY = 1e-3        # stronger than the classifier's 1e-4, see docstring
...
# =====================================================
# LOAD DATA (identical to RF / MLP classifier scripts)
# =====================================================
...
# =====================================================
# NAME -> ALLERGEN_ID MAPPING (identical logic)
# =====================================================
...
# =====================================================
# EMBEDDING MATRIX + COSINE MATRIX (identical to prior scripts)
# =====================================================
...
# =====================================================
# MAP GOLD STANDARD PAIRS TO ALLERGEN IDS (identical logic)
# =====================================================
...
# =====================================================
# GROUP-AWARE PROTEIN-LEVEL SPLIT (identical to RF / MLP classifier --
# same seed + same algorithm on the same inputs reproduces the same
# 1224/310 protein split and the same 55 held-out test pairs)
# =====================================================
...
print("\n==============================")
print("EMBEDDING STANDARDIZATION")
print("==============================")
...
print("Standardization fitted on TRAIN-split protein embeddings only "
      f"({len(train_protein_indices)} proteins).")
...
# =====================================================
# PAIR-LEVEL FIT/VALIDATION SPLIT (within TRAIN pairs, for early stopping)
# =====================================================
...
def build_directed_examples(pairs):
    """Each pair (A, B) -> two directed, standardized (input, target) rows."""
    inputs, targets = [], []
    for p in pairs:
...
# (does the model beat a trivial "predict partner = self" identity map?)
# =====================================================

identity_val_mse = float(np.mean((X_val - Y_val) ** 2))
print(f"\nSanity check -- identity-mapping MSE on val set (predicted_B = A): "
      f"{identity_val_mse:.4f}")
print("(the trained model's val MSE, reported after training, should be "
      "meaningfully LOWER than this for the transformation to be doing "
      "more than trivially copying the input)")
...
class EmbeddingTransformMLP(nn.Module):
    """1280 -> Linear(256) -> ReLU -> Dropout(0.3) -> Linear(128) -> ReLU
    -> Dropout(0.2) -> Linear(1280). No output activation (unbounded,
    standardized embedding-space targets)."""
...
print(f"\nTotal parameters: {n_params:,}  vs.  fit examples: {X_fit.shape[0]} "
      f"(ratio: {n_params / X_fit.shape[0]:.0f} params per example -- see "
      f"overfitting-risk note in the module docstring)")
...
if best_val_mse < identity_val_mse:
    print("-> Model beats the trivial identity baseline (learned a non-trivial "
          "transformation).")
else:
    print("-> WARNING: model does NOT beat the trivial identity baseline -- "
          "the learned transformation may be degenerate on this small dataset.")


# =====================================================
# RETRIEVAL EVALUATION (same 110 queries as RF / MLP classifier)
# =====================================================
...
with torch.no_grad():
    for p in test_positive_pairs:
        ...
        for query_id, target_id, query_name, target_name, family_q, family_t in directions:
            ...
            # cosine similarity between predicted partner embedding and
            # every real ESM-2 embedding in the pool
            mlp_scores = cosine_similarity(
                predicted_raw.reshape(1, -1), embedding_matrix
            )[0]
            ...
            # independent cosine-similarity recomputation, for the
            # cross-script consistency check below
...
# =====================================================
# CROSS-CHECK + LOAD RF / MLP CLASSIFIER SAVED RESULTS
...
def load_other_method(path, hits_prefix, mrr_col, method_label):
    ...
    if len(other_df) != len(retrieval_df):
        print(
            f"\nWARNING: {method_label} retrieval file has {len(other_df)} queries, "
            f"this script has {len(retrieval_df)} -- test sets differ, skipping."
        )
        return {k: float("nan") for k in TOP_K}, float("nan"), None

    merged = retrieval_df.merge(
        other_df[["pair_id", "query_allergen", "cosine_hits_at_1", "cosine_reciprocal_rank"]],
        on=["pair_id", "query_allergen"],
        suffixes=("_this_script", "_other_script"),
    )
    consistent = bool(
        np.allclose(
            merged["cosine_reciprocal_rank_this_script"],
            merged["cosine_reciprocal_rank_other_script"],
        )
    )
...
rf_hits, rf_mrr, rf_consistent = load_other_method(
    RF_RETRIEVAL_RESULTS, "rf", "rf_reciprocal_rank", "Random Forest"
)
...
summary_lines.append("MLP EMBEDDING TRANSFORMATION (Approach B) - SUMMARY")
summary_lines.append("=" * 60)
...
summary_lines.append("Split strategy: identical group-aware protein-level split as "
                      "ml/random_forest_baseline.py and ml/mlp_baseline.py.")
...
if rf_consistent is not None:
    summary_lines.append(f"  Split consistency check vs RF script : {rf_consistent}")
if mlp_clf_consistent is not None:
    summary_lines.append(f"  Split consistency check vs MLP clf   : {mlp_clf_consistent}")
summary_lines.append("")
summary_lines.append("Architecture: 1280 -> Linear(256) -> ReLU -> Dropout(0.3) -> "
                      "Linear(128) -> ReLU -> Dropout(0.2) -> Linear(1280)")
...
summary_lines.append(
    "  -> model BEATS identity baseline (non-degenerate)" if best_val_mse < identity_val_mse
...
header = (
    f"{'Metric':<10}{'Cosine (same 55)':<20}{'Random Forest':<20}"
    f"{'MLP classifier':<20}{'MLP embed. transform':<20}"
)
...
summary_lines.append("")
summary_lines.append(
    "All four methods above are evaluated on the exact same 55 held-out "
    "test pairs / 110 retrieval queries, against the same 1534-protein "
    "candidate pool (subject to the RF/MLP-classifier result files being "
    "present and consistency-checked above)."
)
