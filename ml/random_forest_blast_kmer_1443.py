"""
Random Forest + BLAST + k-mer sastav sekvence (BEZ same_family) - 1443 dataset.

Ne menja random_forest_blast_1443.py ni random_forest_features2_1443.py.

NAMERNO BEZ same_family feature-a: prethodna verzija (features2) sa
same_family je dala MRR skok na 0.31, ali se ispostavilo da je cirkularno --
99.7% SVIH pozitivnih parova u gold standardu (ne samo "Inferred" kategorija,
i "Confirmed/Strong" 98.8%) je unutar iste familije, pa same_family gotovo
savrseno kodira sam label, ne nezavisan biolski signal. Ova verzija testira
koliko STVARNOG napretka daje samo BLAST + kmer (oba ESM-nezavisna, ali
NE izvedena iz cross-reactivity oznake same) na baseline RF.

Feature vektor: 1280 (abs_diff) + cosine + blast_identity + blast_score +
kmer_similarity = 1284 dimenzije.

Izlaz:
    output/random_forest_blast_kmer_model_1443.joblib
    output/random_forest_blast_kmer_retrieval_results_1443.csv
    output/random_forest_blast_kmer_summary_1443.txt
"""
...
MODEL_OUTPUT = OUTPUT_DIR / "random_forest_blast_kmer_model_1443.joblib"
RETRIEVAL_OUTPUT = OUTPUT_DIR / "random_forest_blast_kmer_retrieval_results_1443.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "random_forest_blast_kmer_summary_1443.txt"
...
# NOTE: no family_map here on purpose -- this script deliberately excludes any
# family-derived feature, see module docstring (circularity finding).
...
# blast_score + kmer_similarity = 1284 dims (no same_family)
# =====================================================
...
print("\n==============================")
print("BUILDING FEATURE MATRICES (1284 dims)")
print("==============================")
...
print("\n==============================")
print("TRAINING RANDOM FOREST (BLAST + kmer, no same_family)")
print("==============================")
...
feature_names_tail = ["cosine", "blast_identity", "blast_score", "kmer_similarity"]
...
for k in TOP_K:
    print(f"Hits@{k:<2d}: Cosine={cosine_test_hits[k]:.4f}  RF+BLAST+kmer={rf_hits[k]:.4f}")
print(f"MRR    : Cosine={cosine_test_mrr:.4f}  RF+BLAST+kmer={rf_mrr:.4f}")
...
add("\n" + "=" * 60)
add("RANDOM FOREST + BLAST + KMER, NO SAME_FAMILY (1443) - SUMMARY")
add("=" * 60)
add(f"Feature vector: 1280 abs_diff + cosine + blast_id + blast_score + "
    f"kmer_sim = {X_train.shape[1]} dims (no same_family -- see docstring)")
...
header = f"{'Metric':<10}{'Cosine':<12}{'RF (ESM)':<12}{'RF+BLAST':<12}{'RF+BLAST+kmer':<12}"
add(header)
add("-" * len(header))
for k in TOP_K:
