"""
LayerNorm ablacija za MLP(hadamard): Linear->LayerNorm->ReLU->Dropout umesto
Linear->ReLU->Dropout, na SKRIVENIM aktivacijama posle prvog (jedinog)
skrivenog sloja. RAZLICIT mehanizam od standardize=False odluke -- ona je
globalna z-score standardizacija SIROVIH ulaznih Hadamard feature-a
(dijagnostikovano da unistava signal, ostaje standardize=False u oba
config-a ispod), dok je LayerNorm ovde per-primer normalizacija SKRIVENIH
aktivacija, ne dodaje/ne menja ulazne feature. use_layernorm je nov,
podrazumevano False parametar u ml/pipeline/models/classifiers/mlp.py
(PairMLP) -- aditivna izmena, ne menja ponasanje nijednog postojeceg
skripta koji ga eksplicitno ne trazi.

Dva config-a, ISTI 40 LOCO folda (loco_folds je deterministicna):
  1. baseline -- identicno ml/loco_blast_vs_mlp_hadamard_only_1548.py
  2. +LayerNorm -- isti hidden_dims/dropout/lr/itd, samo use_layernorm=True

Trening SVAKI fold ispocetka (training_eligible_pairs, cist trening, ista
disciplina kao svuda).

Izlaz:
    output/loco_mlp_hadamard_layernorm_ablation_1548_per_query.csv
    output/loco_mlp_hadamard_layernorm_ablation_1548_summary.txt
"""
...
PER_QUERY_OUTPUT = Path("/home/lana/ALERGRAF/output/loco_mlp_hadamard_layernorm_ablation_1548_per_query.csv")
SUMMARY_OUTPUT = Path("/home/lana/ALERGRAF/output/loco_mlp_hadamard_layernorm_ablation_1548_summary.txt")
...
CONFIGS = [
    ("baseline", {**BASE_PARAMS, "use_layernorm": False}),
    ("layernorm", {**BASE_PARAMS, "use_layernorm": True}),
]
    ...
    for fold_idx, (train_pairs, test_pairs, test_ids) in enumerate(folds):
        ...
        for p in test_pairs:
            for query_id, target_id in [(p["id_1"], p["id_2"]), (p["id_2"], p["id_1"])]:
                ...
                all_records.append({
                    "config": config_name, "fold": fold_idx, "pair_id": p["pair_id"],
        ...
        print(f"  fold {fold_idx + 1}/{K_FOLDS} -- blast={fold_df_tmp['blast_rr'].mean():.4f} "
              f"mlp={mlp_mrr_str} ({elapsed/60:.1f} min)", flush=True)
...
print(f"Sve config-e x {K_FOLDS} LOCO folda gotovo za {total_elapsed/60:.1f} min", flush=True)
...
summary_lines = ["=" * 80, f"LOCO ({K_FOLDS} folds) -- MLP(hadamard) LayerNorm ablacija vs BLAST", "=" * 80, "",
...
# Direktno poredjenje baseline vs layernorm (uparen po pair_id+fold)
base = df[df["config"] == "baseline"].dropna(subset=["mlp_rr"])
ln = df[df["config"] == "layernorm"].dropna(subset=["mlp_rr"])
merged_cfg = base.merge(ln, on=["fold", "pair_id"], suffixes=("_base", "_ln"))
delta_ln = merged_cfg["mlp_rr_ln"] - merged_cfg["mlp_rr_base"]
summary_lines.append(f"--- LayerNorm vs baseline (direktno, isti upiti, n={len(merged_cfg)}) ---")
summary_lines.append(f"  Mean MRR delta (layernorm - baseline): {delta_ln.mean():+.4f}")
...
for _ in range(N_BOOTSTRAP):
    ...
    w = resampled["w"].to_numpy()
    d = np.average(resampled["mlp_rr_ln"], weights=w) - np.average(resampled["mlp_rr_base"], weights=w)
    deltas_cfg.append(d)
