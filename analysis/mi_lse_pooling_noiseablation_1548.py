"""
ABLACIJA suma: identicno kao mi_lse_pooling_1548.py, ALI iz treninga
iskljucuje 11 "Mite group 5/7/21" Inferred parova koji ne prolaze WHO2001
kriterijum I za koje literatura (Der p 5/21 vs grupa 7) sugerise da real
IgE unakrsna reaktivnost NIJE potvrdjena ("no relevant serum IgE cross-
reactivity observed" -- za razliku od nsLTP/2S albumin/Lipocalin, gde je
nizak identitet OCEKIVAN i biolski opravdan, pa NISU dirnuti). Test da li
uklanjanje ovih 11 verovatno-sumnjivih parova pomera fitovan tau i
poboljsava held-out MRR na nsLTP/Profilin/PR-10 (bez menjanja bilo cega
drugog -- ista trening/eval podela, isti seed).
...
SUMMARY_OUTPUT = Path("/home/lana/ALERGRAF/output/mi_lse_pooling_noiseablation_1548_summary.txt")

EXCLUDE_MITE_NOISE_PAIR_IDS = {
    "INF1089", "INF1091", "INF1092", "INF1095", "INF1098", "INF1100",
    "INF1103", "INF1107", "INF1109", "INF1112", "INF1113",
}
...
# -------------------------------------------------------
train_positive_pairs = [p for p in dataset.gold_pairs
                         if p.get("family_1") not in TARGET_FAMILIES
                         and str(p.get("pair_id")) not in EXCLUDE_MITE_NOISE_PAIR_IDS]
held_out_positive_pairs = [p for p in dataset.gold_pairs if p.get("family_1") in TARGET_FAMILIES]
n_excluded = sum(1 for p in dataset.gold_pairs if str(p.get("pair_id")) in EXCLUDE_MITE_NOISE_PAIR_IDS)
print(f"\nIskljuceno iz treninga (Mite group 5/7/21 sumnjivi parovi): {n_excluded}")
print(f"Trening pozitivnih parova (bez ciljnih familija, bez mite-sumnjivih): {len(train_positive_pairs)}")
...
summary_text = "\n".join(summary_lines)
print("\n" + summary_text)
results_df.to_csv("/home/lana/ALERGRAF/output/mi_lse_pooling_noiseablation_1548_per_query.csv", index=False)
