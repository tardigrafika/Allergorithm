...
TAU_OUTPUT = Path("/home/lana/ALERGRAF/output/rrf5_cleantrain_lse_tau.txt")
RAW_OUTPUT = Path("/home/lana/ALERGRAF/test/evaluation_results_raw_rrf5_cleantrain.json")
SUMMARY_OUTPUT = Path("/home/lana/ALERGRAF/test/evaluate_rrf5_cleantrain_1548_summary.txt")
...
# (BEZ preostalih Inferred-tier parova -- odluka od 2026-08-29, error_taxonomy
# + study-level bootstrap pokazali da su ti parovi i statisticki nepouzdani I
# 3.5x cesce pogresno rangirani; training_eligible_pairs() ih izbacuje SAMO
# iz treninga, ne iz dataset.gold_pairs/evaluacije. Ovo NIJE LOCO test, gradimo
# produkcioni alat -- poredimo protiv istog patient test-a kao RRF-5 original
# da vidimo da li ciscenje trening signala popravlja ono sto je RRF-5 pokvario.)
# -------------------------------------------------------
train_pairs_clean = training_eligible_pairs(dataset.gold_pairs)
print(f"\nTreniram produkcioni LSE model (BEZ preostalih Inferred: {len(train_pairs_clean)}/"
      f"{len(dataset.gold_pairs)} gold parova)...", flush=True)
...
for i, p in enumerate(train_pairs_clean):
    S = pair_similarity_matrix(p["id_1"], p["id_2"])
    if S is not None:
        train_sims.append(S.flatten())
        train_labels.append(1.0)
    if (i + 1) % 300 == 0:
        print(f"    pozitivnih obradjeno: {i+1}/{len(train_pairs_clean)}", flush=True)
